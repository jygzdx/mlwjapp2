...
import easeui.choisepictures.ImgFileListActivity;
...
public class EaseChatFragment extends EaseBaseFragment implements EMEventListener {
    ...
    protected static final int REQUEST_CODE_HD = 4;
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            ...
            titleBar.setRightImageResource(R.drawable.mlx_group_detail);
        } else {
            titleBar.setRightImageResource(R.drawable.mlx_group_detail);
            if (chatType == EaseConstant.CHATTYPE_GROUP) {
                ...
                text_notice.setText(group.getDescription());*/
                // 监听当前会话的群聊解散被T事件
...
//                    }
                    ...
                    List<String> l = bundle.getStringArrayList("files");
                    if (l != null) {
                        for (String url : l
                                ) {
...
//                selectHDPicFromLocal(); // 图库选择图片 高清
                    //用自己的可以多选的照片 高清
                    Intent i = new Intent();
                    i.setClass(getActivity(), ImgFileListActivity.class);
                    startActivityForResult(i, REQUEST_CODE_HD);
                    break;
                default:
                    break;
            }
    ...
    protected void sendHDImageMessage(String imagePath) {
    ...
    protected void sendHDPicByUri(Uri selectedImage) {
        ...
        if (cursor != null) {
            ...
            sendHDImageMessage(picturePath);
        } else {
            File file = new File(selectedImage.getPath());
            if (!file.exists()) {
                Toast toast = Toast.makeText(getActivity(), R.string.cant_find_pictures, Toast
            ...
            sendHDImageMessage(file.getAbsolutePath());
        }
     ...
     * 从图库获取高清图片
     */
    protected void selectHDPicFromLocal() {
        Intent intent;
        if (Build.VERSION.SDK_INT < 19) {
            intent = new Intent(Intent.ACTION_GET_CONTENT);
            intent.setType("image/*");
        ...
        startActivityForResult(intent, REQUEST_CODE_HD);
    }
