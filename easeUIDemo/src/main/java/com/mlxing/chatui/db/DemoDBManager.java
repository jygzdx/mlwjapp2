...
public class DemoDBManager {
    ...
    synchronized public void saveContact(EaseUser user){
        ...
        if (user.getNick()==user.getUsername()){
            return;
        }
     ...
     * 保存陌生人列表
     */
    synchronized public void saveStrangerList(List<EaseUser> strangerList){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            for (EaseUser user : strangerList) {
                ContentValues values = new ContentValues();
                values.put(UserDao.STRANGER_NAME_ID, user.getUsername());
                if(user.getNick() != null)
                    values.put(UserDao.STRANGER_NAME_NICK, user.getNick());
                if(user.getAvatar() != null)
                    values.put(UserDao.STRANGER_NAME_AVATAR, user.getAvatar());
                db.replace(UserDao.STRANGER_TABLE_NAME, null, values);
            }
        }
    }
     ...
     * @return
     */
    synchronized public Map<String, EaseUser> getStrangerList() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Map<String, EaseUser> users = new HashMap<String, EaseUser>();
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.STRANGER_TABLE_NAME /* + " desc" */, null);
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(UserDao.STRANGER_NAME_ID));
                String nick = cursor.getString(cursor.getColumnIndex(UserDao.STRANGER_NAME_NICK));
                String avatar = cursor.getString(cursor.getColumnIndex(UserDao.STRANGER_NAME_AVATAR));
                EaseUser user = new EaseUser(username);
                user.setNick(nick);
                user.setAvatar(avatar);
                users.put(username, user);
        ...
        Log.i("demodb", "getusers: "+users.values());
        return users;
    }
