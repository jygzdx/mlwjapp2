...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		user = DemoHelper.getInstance().getUserInfo(username);
		if (user==null){
			/*HttpUtil.getInstance().getUserInfo(username, new HttpUtil.InfoCallBack() {
				@Override
				public void onSuccess(List<EaseUser> result) {
					user = result.get(0);
					if (user!=null&&!VoiceCallActivity.this.isFinishing()){
						nickTextView.setText(user.getNick());
//						int avatarResId = Integer.parseInt(user.getAvatar());
						Glide.with(VoiceCallActivity.this).load(user.getAvatar()).into(swing_card);
					}
				}
			});*/
